...
function selectMatch(match, constraints, selected) {
  ...
  inspector.innerHTML = `<h3>Why this ranked first</h3>
    <h4>Key feature values</h4><ul class="features">
      ...
      ${featureRow("Delivery ETA", `${feature.delivery_time_hours.toFixed(1)} hours`)}</ul>
    <h4>Applied constraints</h4><ul class="constraints">${constraints.map((item) => `<li><span>${item}</span><b>Pass</b></li>`).join("")}</ul>
    <p class="reason">${title(match.provider_id)}. ${match.reasons.join(". ")}.</p>`;
}
